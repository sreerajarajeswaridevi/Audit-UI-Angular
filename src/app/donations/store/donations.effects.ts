...
export class DonationsEffects {
  ...
  query$ = this.actions$.pipe(
    ...
    switchMap(([, user]: any) => this.donationsService.get(user.uid)
      .pipe(
        map((data: any) => {
  ...
  added$ = this.actions$.pipe(
    ...
    withLatestFrom(this.store.pipe(select(getUser))),
    switchMap(([payload, user]: any) => this.donationsService.add(payload.customer, user.uid))
  );
  ...
  edit$ = this.actions$.pipe(
    ...
    withLatestFrom(this.store.pipe(select(getUser))),
    switchMap(([payload, user]: any) => this.donationsService.update(payload.customer, user.uid)
    .pipe(
      catchError( error => {
      return of(new fromDonations.DonationsError({ error }));
    }))
  ...
  delete$ = this.actions$.pipe(
    ...
    withLatestFrom(this.store.pipe(select(getUser))),
    switchMap(([payload, user]: any) => this.donationsService.delete(payload.customer, user.uid))
  );
}
